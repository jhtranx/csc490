...
shared_ptr<regionData> readCSVLineDemog(std::string theLine) {
   visitorCombineCounty strip_county_object_ = visitorCombineCounty();

   std::stringstream ss (theLine);

   string unstripped_county_ = getField(ss);
   string stripped_county_ = strip_county_object_.stripCounty(unstripped_county_);
   ...
   return make_shared<demogData> (
      ...
      race_eth_, 
      stripped_county_, state_, tot_population_ct_
   );
}
