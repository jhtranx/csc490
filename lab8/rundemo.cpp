...
std::vector<string> dataProjSetUp() {
	...
	sort(theData.begin(), theData.end(), [](shared_ptr<demogData> lhs, 
		shared_ptr<demogData> rhs) {
			return lhs->getMedIncome() > rhs->getMedIncome();
	}); 

	for (auto &obj : theData) {	
		string strippedCounty = theAnswers.stripCounty(obj->getCounty());
		countyToMedIncome.push_back(strippedCounty+obj->getState());
	}
...
// 		std::cout << " added 2nd style ... \n";
		...
		// state data
		{
			...
			for (int l = 0; l < colorMap.size(); l++) {
				cout << "HERE: " << "section"+to_string(l) << endl;
